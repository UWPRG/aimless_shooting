...
class GeneratePlumed:
    ...
    def add_angles(self, atoms, periodic=False):
        ...
        if len(atoms) >= 3:
            ...
            for j_counter, j in enumerate(triples):
                self.string += 'a{}: ANGLE ATOMS={},{},{} '.format(
                        j_counter, j[0], j[1], j[2])
                if not periodic:
                    self.string += 'NOPBC\n'
                elif periodic:
                    self.string += '\n'
    ...
    def add_distances(self, atoms, diff_of_dist=True, periodic=False,
                      components=False):
        ...
        if len(atoms) >= 2:
            ...
            for i_counter, i in enumerate(pairs):
                self.string += 'd{}: DISTANCE ATOMS={},{} '.format(
                        i_counter, i[0], i[1])
                if not periodic:
                    self.string += 'NOPBC'
                elif periodic:
        ...
        if len(group_name) == len(atoms):
            for group_counter, group in enumerate(group_name):
                self.string += '{}: GROUP ATOMS={}\n'.format(
                        group, atoms[group_counter])
        else:
            print("Length of group names must equal length of atoms")

    def add_coordination(self, group_a, group_b, r_0, nl_cutoff=[1.0],
                         nlist=True, nn=6, mm=0, d_0=0.0):
        """
        Parameters
        ----------
        groupa: str
        ...
        r_0 : int
            ...
            (see https://plumed.github.io/doc-v2.4/user-doc/html
                 /_c_o_o_r_d_i_n_a_t_i_o_n.html)
        nl_cutoff : int
    ...
    def add_puckering(self, atoms):
        """
        Given a list of 5 or 6 atoms, or a group_name, will create a puckering
        CV
    ...
    def add_committor(self, basins, basin_CV, limit):
        ...
        basin_CV : list of strings
            List of variables that correspond to the CVs to be monitored for
            each basin. Only accepts 2-3 CVs as an input.
            Example: ['d1','d2']
